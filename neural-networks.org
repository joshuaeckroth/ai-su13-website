...
with its current weights $w_{ji}$. We do not want to adjust the
weights if the output is correct. If the output is 0 and we should
have yielded a 1, then the weights are too low, and should be adjusted
up. If the output is a 1 and we wanted a 0, the weights are too
high. Of course, we only want to adjust a tiny bit, so we do not
oscillate. Thus, we have the following adjustment for each weight:

$$w'_{ji} = w_{ji} + \alpha (d_j-y_j)x_i,$$

where $w'_{ji}$ is the new weight and $\alpha$ is a small number (0.1,
...
&=& \frac{\partial}{\partial w_{ji}} (d_j - f(s_j))^2 \\
&=& 2 (d_j - f(s_j)) \frac{\partial}{\partial w_{ji}} (d_j - f(s_j)) \qquad \text{chain rule} \\
&=& -2 (d_j - f(s_j)) f'(s_j) \frac{\partial}{\partial w_{ji}} s_j \qquad \text{chain rule}\\
&=& -2 (d_j - f(s_j)) f'(s_j) \frac{\partial}{\partial w_{ji}} \sum_{i'=1}^k x_{i'} w_{ji'} + w_{j0} \qquad \text{expand } s_j\\
&=& -2 (d_j - f(s_j)) f'(s_j) x_{i} \qquad \text{picks out } x_{i} w_{ji} \text{, whose derivative is } x_{i}
\end{eqnarray}
...
$$\frac{\partial L_j}{\partial w_{ji}} = \alpha (d_j - f(s_j)) f(s_j) (1 - f(s_j)) x_{i}$$

So our weight update is the following, with $p_j = f(s_j)$:

$$w'_{ji} = w_{ji} + \alpha (d_j - p_j) p_j (1 - p_j)) x_{i}$$
...
\Delta_j = 
\begin{cases}
f'(s_j) (d_j - p_j) & \text{if perceptron } j \text{ is an output node} \\
